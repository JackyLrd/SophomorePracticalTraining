...
public class Jigsaw {
	...
	private void estimateValue(JigsawNode jNode) {
		int s = 0;
		s = 0 * wrongloc(jNode) + 1 * wrongtocorrectsum(jNode) + 1 * successivelywrong(jNode);
		//s = 1;
		jNode.setEstimatedValue(s);
	}

	private int wrongloc(JigsawNode jNode)
	{
		int s = 0;
		int[] cur = jNode.getNodesState();
		int[] end = getEndJNode().getNodesState();
	...
	private int wrongtocorrectsum(JigsawNode jNode)
	{
		int s = 0;
		int[] cur = jNode.getNodesState();
		int[] end = getEndJNode().getNodesState();
		...
		for(int index = 1 ; index <= dimension * dimension; index++)
		{
			if(cur[index] != 0 && cur[index] != end[index])
			{
				int currow = (int) (index - 1) / dimension;
				int curcol = (int) (index + 4) % dimension;
				for(int i = 1; i <= dimension * dimension; i++)
				{
					if(end[i] == cur[index])
					{
						int endrow = (int) (i - 1) / dimension;
						int endcol = (int) (i + 4) % dimension;
						s += Math.abs(currow - endrow) + Math.abs(curcol - endcol);
						break;
					}
				}
			}
	...
	private int successivelywrong(JigsawNode jNode)
	{
		int s = 0;
		int dimension = JigsawNode.getDimension();
		for(int index = 1 ; index < dimension * dimension; index++)
